...
public class TransactionListQueryModelMaterializerUnitTest
{
    ...
    public void Should_Create_Model_With_Right_Data()
    {
        ...
        var queryModel = _materializer.Materialize(
            transaction,
            cardList
        );
    ...
    private void AssertProperties(
        TransactionListQueryModel queryModel,
        Transaction transaction,
        CardListQueryModel cardList
    )
    {
        Assert.Equal(
            queryModel.Id,
        ...
        Assert.Equal(
            queryModel.CurrencyCode,
            transaction.Charge.CurrencyCode
        );
        Assert.Equal(
            queryModel.ChargeDate,
        ...
        Assert.Equal(
            queryModel.CardHolder,
            cardList.CardHolder
        );
        Assert.Equal(
            queryModel.CardNumber,
            cardList.Number
        );
    }
}