...
public class
    GetTransactionListQueryHandler : IQueryHandler<GetTransactionListQuery, IEnumerable<TransactionListQueryModel>>
{
    ...
    public async Task<IEnumerable<TransactionListQueryModel>> HandleAsync(GetTransactionListQuery query)
    {
        var result = _readDbContext
            ...
            .AsQueryable()
            .WhereIf(
                !string.IsNullOrEmpty(query.CardNumber),
                x => x.CardNumber == query.CardNumber
            )
            .WhereIf(
                !string.IsNullOrEmpty(query.CardHolder),
                x => x.CardHolder.Contains(query.CardNumber)
            )
            .WhereIf(
                query.ChargeDate.HasValue,
                x => x.ChargeDate == query.ChargeDate
            )
            .WhereIf(
                query.BetweenAmount.HasValue,
                x => x.Amount >= query.BetweenAmount && x.Amount <= query.BetweenAmount
            );
